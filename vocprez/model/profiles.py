...
reduced_rdf_types = Renderer.RDF_MEDIA_TYPES
reduced_rdf_types.remove('text/n3')
profile_skos = Profile(
    ...
    "via the Web.",
    mediatypes=["text/html"] + reduced_rdf_types,
...
profile_describe = Profile(
    "https://www.w3.org/TR/skos-reference/",
    label="Describe",
    comment="All simple properties of an object",
    mediatypes=["text/html"] + reduced_rdf_types,
    default_mediatype="text/html",
    languages=["en"],  # default 'en' only for now
    default_language="en",
...
profile_vocpub = Profile(
    ...
    mediatypes=["text/html", "application/json"] + reduced_rdf_types,
    default_mediatype="text/html",
    languages=["en"],  # default 'en' only for now
    default_language="en",
)
